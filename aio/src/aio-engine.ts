...
import { WorkPackageData, PRMetadata, AIOState, Config, TemplateData } from './types.js';
...
export class AIOEngine {
  ...
  private async prepareWorkPackageData(issueId: string): Promise<WorkPackageData> {
    ...
    if (existsSync(prMetadataPath)) {
      const prMetadata: PRMetadata = JSON.parse(readFileSync(prMetadataPath, 'utf-8'));
      pullRequest = await this.githubService.getPullRequest(prMetadata.id);
      
      // Get comments from both issue and PR
      const issueComments = await this.githubService.getIssueComments(issue.number);
      const prComments = await this.githubService.getPullRequestComments(prMetadata.id);
  ...
  private determineState(data: WorkPackageData): AIOState {
    ...
    console.log(`No matching state found for issue #${data.issue.number}, defaulting to BOOTSTRAP`);
    return 'BOOTSTRAP';
  }
  ...
  private async handleBootstrap(data: WorkPackageData): Promise<void> {
    ...
    await this.githubService.removeLabelFromIssue(data.issue.number, 'ready-for-agent');
    await this.githubService.addLabelToIssue(data.issue.number, 'plan-proposed');
    await this.githubService.addLabelToIssue(data.issue.number, 'locked');

    // Create folder
    mkdirSync(data.workPackageName, { recursive: true });

    // Write TASK.md and cost.md
    ...
    // Commit and push
    await this.commitAndPush(data, 'Bootstrap work package');
  ...
  private async handlePlanApproved(data: WorkPackageData): Promise<void> {
    ...
    await this.githubService.removeLabelFromIssue(data.issue.number, 'plan-approved');
  ...
  private async handleReview(data: WorkPackageData): Promise<void> {
    ...
    // Check CI status
    const hasFailedChecks = data.checks.some(check => check.conclusion === 'failure');
  ...
  private async handleReadyToMerge(data: WorkPackageData): Promise<void> {
    ...
    await this.githubService.removeAllLabelsFromPullRequest(data.pullRequest.number);
    ...
    if (existsSync(costPath)) {
      ...
      await this.githubService.addCommentToIssue(data.issue.number, costContent);
    }
  ...
  private createTemplateData(data: WorkPackageData, qaContent?: string): TemplateData {
    return {
      ...data,
      qaContent
    };
  ...
  private async commitAndPush(data: WorkPackageData, message: string): Promise<void> {
